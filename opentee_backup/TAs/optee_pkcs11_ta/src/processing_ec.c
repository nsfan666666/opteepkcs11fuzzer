					     ...
					     size_t tee_size)
{
	...
	size_t hsize = 0;
	enum pkcs11_rc rc = PKCS11_CKR_GENERAL_ERROR;
	OT_LOG(LOG_ERR, "Inisde tee2pkcs_ec_attributes 1");

	rc = tee2pkcs_add_attribute(priv_head, PKCS11_CKA_VALUE,
				    tee_obj, TEE_ATTR_ECC_PRIVATE_VALUE);
	if (rc)
		goto out;
	OT_LOG(LOG_ERR, "Inisde tee2pkcs_ec_attributes 2");
	...
	OT_LOG(LOG_ERR, "Inisde tee2pkcs_ec_attributes 3");
	...
	if (qsize < 0x80) {
		...
		OT_LOG(LOG_ERR, "Too long DER value");
				...
				struct obj_attrs **priv_head)
{
	...
	OT_LOG(LOG_ERR, "Inisde generate_ec_keys 1");

	if (get_attribute_ptr(*pub_head, PKCS11_CKA_EC_PARAMS,
			      &a_ptr, &a_size) || !a_ptr) {
		OT_LOG(LOG_ERR, "No EC_PARAMS attribute found in public key");
	...
	res = TEE_AllocateTransientObject(TEE_TYPE_ECDSA_KEYPAIR, tee_size,
					  &tee_obj);
	if (res) {
		OT_LOG(LOG_ERR, "Transient alloc failed with %#"PRIx32, res);
		return tee2pkcs_error(res);
	}
	OT_LOG(LOG_ERR, "Inisde generate_ec_keys 2");
	...
	OT_LOG(LOG_ERR, "Inisde generate_ec_keys 3");
	...
	if (rc)
		goto out;
	OT_LOG(LOG_ERR, "Inisde generate_ec_keys 4");
...
size_t ecdsa_get_input_max_byte_size(TEE_OperationHandle op)
{
	...
	case TEE_ALG_ECDSA_P521:
		return 66;
	default:
		//printf("Unexpected ECDSA algorithm %#"PRIx32, info.algorithm);
		return 0;
	}
}
