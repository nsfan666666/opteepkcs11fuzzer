...
int load_lib(char *path, main_loop_cb *callback)
{
	...
	*(void **)(callback) = dlsym(lib, "lib_main_loop");
	...
	if (err != NULL || !*callback) {
		OT_LOG(LOG_ERR, "Failed to find lib_main_loop");
		ret = -1;
	}
...
int main(int argc, char **argv)
{
	...
	if (control_params.launcher_pid == -1) {
		...
		prctl(PR_SET_PDEATHSIG, SIGKILL); // ! changed from SIGTERM to SIGKILL (else Launcher process might remain after engine shutdown/restart)
	} else {
