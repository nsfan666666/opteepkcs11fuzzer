...
static bool __attribute__((constructor)) storage_ext_init()
{
	...
	if (tee_storage_dir != NULL) {
		...
		#ifndef ANDROID
		tee_storage_dir = getenv("HOME"); // ! Environmen variable must be passed via execle or similar in AFL
		#else
...
static bool get_uuid(char *uuid)
{
	...
	uuid[TEE_UUID_LEN_HEX - 1] = '\0'; // ! kz: changed from TEE_UUID_LEN_HEX to TEE_UUID_LEN_HEX - 1 (otherwise it will cause SEGV)
}
