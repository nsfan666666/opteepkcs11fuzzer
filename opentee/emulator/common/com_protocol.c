...
ssize_t send_fd_pid(int fd, int pid)
{
	int out[1] = {pid};
	ssize_t res;

	if ((res = write(fd, out, 4) != 4)) 
		OT_LOG(LOG_ERR, "send TA PID failed (res=%zd), reason: %s", res, strerror(errno));
	
	return res;
}

void send_afl_pid(int pid) 
{
	if (ta_st_fd < 0) 
		OT_LOG(LOG_ERR, "send_afl_pid: Bad ta_st_fd=%d", ta_st_fd);

	if (pid < 0) 
		OT_LOG(LOG_ERR, "send_afl_pid: Bad TA PID=%d", pid);

	send_fd_pid(ta_st_fd, pid);
}

ssize_t send_fd_status(int fd, int status) 
{
	int out[1] = {status};

	ssize_t res;
	if ((res = write(fd, out, 4) != 4)) 
		OT_LOG(LOG_ERR, "send TA status failed (res=%zd), reason: %s", res, strerror(errno));

	return res;
}

void send_afl_status(int status)
{
	if (ta_st_fd < 0) 
		OT_LOG(LOG_ERR, "send_afl_status: Bad ta_st_fd=%d", ta_st_fd);
	
	if (status < 0) 
		OT_LOG(LOG_ERR, "send_afl_status: Bad TA status=%d", status);

	send_fd_status(ta_st_fd, status);
}
