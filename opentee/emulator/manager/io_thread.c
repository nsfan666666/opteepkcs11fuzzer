...
static void send_msg(proc_t send_to, void *msg, int msg_len)
{
	...
	// kz sends message to different proc_type (process types) e.g. CA/TA/session similar to GP API format
...
void handle_out_queue(struct epoll_event *event)
{
	...
	// kz: remove msg from outbound list and send it to the receiver stored in the msg
