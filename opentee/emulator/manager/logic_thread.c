...
static void open_session_query(struct manager_msg *man_msg)
{
	...
	if (conn_ta) {
		...
		// ! TA keep-alive will not create new TA instance, instead a new session is created for the current TA process (I close keep-alive flag in open_tee_conf.c under gp folder)
	
// if ((getenv("AFL"))) {
// 		OT_LOG(LOG_ERR, "KZ: Error: Single-instance is active for TA, and should be disabled!");
// 		close(ta_st_fd);
// } // END AFL
		...
		// ! send TA PID to AFL (create a new TA process) (it's now done in Launcher instead to avoid delays for the manager to receive the PID)
...
if ((getenv("AFL"))) {
	...
	// ! send TA PID to AFL (SIGCHILD occured)
	
	OT_LOG(LOG_DEBUG,"|----> proc_changed_state, status=%d <----|", status);
	OT_LOG(LOG_DEBUG,"|----> WIFSIGNALED(status)=%d <----|", WIFSIGNALED(status));

	if (ta_st_fd != -1) {
		send_afl_status(status);	
	}
