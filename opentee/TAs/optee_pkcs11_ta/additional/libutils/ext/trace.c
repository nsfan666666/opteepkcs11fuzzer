...
// ! Replace printk (kernel print function) with user space print
#include <stdio.h> // #include <printk.h>
#include <stdarg.h>
#include <string.h>
#include <trace.h>
#include <util.h>
...
static int print_thread_id(char *buf, size_t bs)
{
	...
	if (thread_id >= 0)
		return snprintf(buf, bs, "%0*d ", num_thread_digits, thread_id); // ! replaced snprintk with snprintf
	else
		return snprintf(buf, bs, "%*s ", num_thread_digits, ""); // ! replaced snprintk with snprintf
}
...
static int print_core_id(char *buf, size_t bs)
{
	...
	if (core_id >= 0)
		return snprintf(buf, bs, "%0*u ", num_digits, core_id); // ! replaced snprintk with snprintf
	else
		return snprintf(buf, bs, "%s ", qm); // ! replaced snprintk with snprintf
}
		   ...
		   const char *fmt, va_list ap)
{
	...
	res = snprintf(buf, sizeof(buf), "%c/", trace_level_to_string(level, level_ok));// ! replaced snprintk with snprintf
	if (res < 0)
		return;
	boffs += res;

	/* Print the location, i.e., TEE core or TA */
	res = snprintf(buf + boffs, sizeof(buf) - boffs, "%s:", trace_ext_prefix); // ! replaced snprintk with snprintf
	if (res < 0)
		return;
	boffs += res;
	...
	if (level_ok && (BIT(level) & CFG_MSG_LONG_PREFIX_MASK)) {
		...
		if (function) {
			res = snprintf(buf + boffs, sizeof(buf) - boffs, "%s:%d ", function, line); // ! replaced snprintk with snprintf
			if (res < 0)
				return;
			boffs += res;
		}
	...
	res = vsnprintf(buf + boffs, sizeof(buf) - boffs, fmt, ap); // ! replaced vsnprintk with vsnprintf
	if (res > 0)
		boffs += res;
...
static int __printf(2, 3) append(struct strbuf *sbuf, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	len = vsnprintf(sbuf->ptr, left, fmt, ap); // ! replaced vsnprintk with vsnprintf
	va_end(ap);
	if (len < 0) {
		/* Format error */
		return 0;
