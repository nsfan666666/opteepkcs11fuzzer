...
pkcs2tee_algorithm(uint32_t *tee_id, uint32_t *tee_hash_id,
		   enum processing_func function __attribute__((unused)),
		       ...
		       struct pkcs11_object *obj)
{
	...
	if (hash_algo) {
		...
		if (res) {
			OT_LOG(LOG_ERR, "TEE_AllocateOp. failed %#"PRIx32" %#"PRIx32,
			     hash_algo, hash_mode);
				   ...
				   enum processing_func function)
{
	...
	enum pkcs11_rc rc = PKCS11_CKR_GENERAL_ERROR;
	TEE_Result res = TEE_ERROR_GENERIC;
	enum pkcs11_class_id __attribute__((unused)) class = get_class(obj->attributes);
	enum pkcs11_key_type type = get_key_type(obj->attributes);
