		       ...
		       TEEC_SharedMemory *io3, size_t *out3_size)
{
	...
	// * if the TA has accepted the invocation (CKR_OK) or rejected it due to too small shm (CKR_BUFFER_TOO_SMALL)
	...
	// if (ta_rc == CKR_OK) {	
	// 	printf("TA has accepted the invocation :)\n");
		
	// 	if(*out2_size > 0) {
	// 		printf("received %zu bytes of data from TA via io2:\n", op.params[2].memref.size);
	// 		print_hex(op.params[2].memref.parent->buffer, op.params[2].memref.size);
	// 	}

	// 	if(*out3_size > 0) {
	// 		printf("received %zu bytes of data from TA via io3:\n", op.params[3].memref.size);
	// 		print_hex(op.params[3].memref.parent->buffer, op.params[3].memref.size);
	// 	}
	// }
