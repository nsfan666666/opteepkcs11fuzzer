...
// ! Replace printk (used in trace.c) with syslog instead
#include <syslog.h>
...
#define trace_printf_helper(level, message_type, message, ...) \
	syslog(level, DBG_LOCATION message, __FILE__, __func__, __LINE__, message_type, ##__VA_ARGS__)
...
#define EMSG(...)   (void)0
#else
#define EMSG(message, ...)   syslog(LOG_ERR, "[EMSG] " message, ##__VA_ARGS__)
#endif
...
#define IMSG(...)   (void)0
#else
#define IMSG(message, ...)   syslog(LOG_INFO, "[IMSG] " message, ##__VA_ARGS__)
#endif
...
#define DMSG(...)   (void)0
#else
#define DMSG(message, ...)   syslog(LOG_DEBUG, "[DMSG] " message, ##__VA_ARGS__)
#endif
...
#define trace_printf_helper_raw(level, message_type, message, ...) \
	syslog(level, DBG_LOCATION message, NULL, NULL, 0, message_type, ##__VA_ARGS__)
...
#else
#define IMSG_RAW(message, ...)   syslog(LOG_INFO, "[IMSG_RAW] " message, ##__VA_ARGS__)
#endif
...
#else
#define DMSG_RAW(message, ...)   syslog(LOG_DEBUG, "[DMSG_RAW] " message, ##__VA_ARGS__)
#endif
